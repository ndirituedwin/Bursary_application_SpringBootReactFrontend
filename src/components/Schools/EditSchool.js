...
import { fetchschoolcategories,fetchcounties, fetchschooltypes, savesecondaryschool,fetchsecschoolById } from '../../constants/ApiUtils';
import { useParams } from 'react-router-dom'

export default function EditSchool() {
   const params=useParams();
   console.log("logging school id",params.schoolId)
    ...
    useEffect(() => {
     ...
     fetchsecschoolById(data).then((resp)=>{
         if(resp){
           setSchooldata({
            code:resp.code,
            school:resp.school,
            ...
            typeId:resp?.type?.id,
            countyCode:resp?.county?.code,
            countyId:resp?.county?.id
           })
         }
    // setSchooldata(resp)
    console.log("fetched n",resp)
    console.log("fetched nnnnnnn",schooldata)
    setSchooldataloading(false)
      console.log("logging school data ",schooldata.id)
     }).catch((error)=>{
         setschooldataerror(error)
       setSchooldataloading(false)
    })
    }, [params.schoolId]) 
      ...
      if(!schoolcategories.length>0){
       allschoolcategories=<option>No data available</option>
        ...
        allschoolcategories=schoolcategories?.map((categ,idx)=>{
          // return <><option key={idx} value={categ.id}  selected={categ.id===schooldata?.category?.id}>{categ.category}</option></>
    ...
    return (
        ...
        <section className="content-header">
            ...
            <section className="content">
             ...
             {/* <div className="col-md-10">{schooldataerror && <span className="help-block text-danger card-title">{schooldataerror}</span>}</div> */}
                 ...
                 <div className="card card-default">
                   <div className="card-header">
                    <p> <h3 className="card-title">Sec and special schools</h3></p>
                        ...
                        <Link to={ROUTES.SHOWALLSCHOOLS} class="btn btn-success float-right">Back</Link>
