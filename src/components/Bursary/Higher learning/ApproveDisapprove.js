...
import $ from 'jquery'
import { gethighereducationbursaryapplicationstoupate, updatehigherstudentisapprovedstatus,higherlearningapprovedisapprovearray } from '../../../constants/ApiUtils';
export default function ApproveDisapprove() {
    ...
    const fetchbursaryapplications=async ()=>{
        try {
          const response = await gethighereducationbursaryapplicationstoupate()
          if (response) {
      ...
      const handleapproveordissapprove=(event,isapproved,applid)=>{
        ...
        updatehigherstudentisapprovedstatus(data).then((resp)=>{
 ...
 const handleglobalapprove=(e)=>{
     ...
     if(!ids.length>0){
         ...
         higherlearningapprovedisapprovearray(ids).then((resp)=>{
                  if(resp){
